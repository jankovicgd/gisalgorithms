...
# Listing 2.8: The even-odd algorithm for the Point-in-polygon test
...
def pip_cross(point, pgon):
    ...
    for j in range(numvert - 1):
        ...
        if yflag1 != yflag2:        # both sides of half-line
            ...
            if xflag2 == xflag1:    # both points on right side
                if xflag1:
                    crossing += 1
...
if __name__ == "__main__":
    ...
    print("Point {} is {}".format(point, inout(pip_cross(point, ppgon)[0])))
    point = Point(10, 20)
    print("Point {} is {}".format(point, inout(pip_cross(point, ppgon)[0])))
    point = Point(20, 40)
    print("Point {} is {}".format(point, inout(pip_cross(point, ppgon)[0])))
